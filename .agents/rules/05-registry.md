...
| `finora-user` | 8085 | PostgreSQL/Neon Project `finora-user`, DB `neondb`, default role của project; Redis nội bộ 6381 (OTP storage, rate limiting) |
| `finora-notification` | 8086 | REST (nhận từ finora-user qua Feign) + SMTP; chưa có DB |
| `finora-ai` | 8000 | FastAPI; chưa có DB |
| Keycloak | 8180 | OIDC; PostgreSQL 17 database/credential/volume riêng, không publish DB port |
| ~~Kafka~~ | ~~9092~~ | Chưa dùng — notification giao tiếp qua Feign HTTP |
...
| User Redis | 6381 host local; 6379 nội bộ | Redis 7, volume riêng của User (OTP TTL, brute-force rate limit) |
...
Consumer group hiện có: `payment-group`, `blockchain-group`.

Kafka topic hiện có: chưa có — notification dùng Feign HTTP thay Kafka. Khi cần async sẽ bổ sung lại.
