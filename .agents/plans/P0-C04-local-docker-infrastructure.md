...
Ngày 2026-08-03, Thái thay đổi hạ tầng database của các module mình sở hữu:

- Loan, Payment và Blockchain dùng PostgreSQL 17.
...
- User/Keycloak của Hải tiếp tục MySQL; Investment tiếp tục MongoDB cho tới khi Hải quyết định khác.
...
```text
core
├── keycloak-db (MySQL 8.4, owner Hải, internal only)
├── keycloak
├── zookeeper
└── kafka
...
user
└── user-mysql (owner Hải, host 13308)

investment
└── investment-mongo (owner Hải, host 27018)
...
- Thái sửa datasource, driver, migration, Testcontainers và config Loan/Payment/Blockchain.
- Không sửa source/config User, Investment, AI hoặc Notification của Hải.
...
`All` bật các storage mới cùng storage của Hải nhưng không thay đổi engine/credential contract module Hải.
...
- `finora-loan/payment/blockchain` POM và `application.yml`.
...
- PostgreSQL volume mới và MySQL volume cũ độc lập; không xóa volume cũ tự động.
...
| ID | Tiêu chí |
|---|---|
| AC-01 | Loan/Payment/Blockchain dùng PostgreSQL JDBC, không còn MySQL driver trong module Thái |
| AC-02 | Loan V1/V2 migrate và Hibernate validate trên PostgreSQL 17 |
| AC-03 | Payment/Blockchain dùng Flyway + validate, không `ddl-auto:update` |
| AC-04 | Ba Docker PostgreSQL có container/volume/port/runtime role riêng |
| AC-05 | Scope Loan/Payment/Blockchain/All resolve đúng service và secret |
| AC-06 | Neon guide có Project riêng, `neondb`/default role, IntelliJ, DBeaver, quota và security |
| AC-07 | User/Keycloak MySQL và Investment Mongo không bị đổi |
...
## 10. Bằng chứng cần điền khi bàn giao

- Compose config cho tất cả profile.
- Loan Maven verify và PostgreSQL Testcontainers.
- Payment/Blockchain compile và PostgreSQL Testcontainers verify.
- Smoke Docker khi daemon sẵn sàng.
- Secret scan, stale MySQL reference audit và link validation.
- Known limitation nếu Neon/Docker không được test trực tiếp.
