...
public interface BorrowerCreditProfileRepository extends JpaRepository<BorrowerCreditProfile, Long> {
    ...
    @Query(value = """
            INSERT INTO borrower_credit_profiles (
                ...
                :borrowerId, false, 0, 0, 0, 'NO_HISTORY',
                'FINORA_INTERNAL_CREDIT_V1', 0, 'SYSTEM', 'SYSTEM', :now, :now
            )
            ON CONFLICT (borrower_id) DO NOTHING
            """, nativeQuery = true)
    int ensureNoHistory(@Param("borrowerId") String borrowerId, @Param("now") Instant now);
}
