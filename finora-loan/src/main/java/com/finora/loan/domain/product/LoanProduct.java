package com.finora.loan.domain.product;
...
public class LoanProduct {
    ...
    public void markCoreSynced(Long mappingId, String actorId, Instant now) {
        if (coreSyncStatus != CoreSyncStatus.PENDING) {
            throw new IllegalStateException("Chỉ Product PENDING mới được xác nhận đồng bộ core");
        }
        if (mappingId == null) {
    ...
    public void markCoreSyncFailed(String actorId, Instant now) {
        if (coreSyncStatus != CoreSyncStatus.PENDING) {
            throw new IllegalStateException("Chỉ Product PENDING mới được ghi nhận lỗi đồng bộ core");
        }
    ...
    /** Chỉ công bố Product đã sync Fineract; version ngăn giao diện cũ kích hoạt nhầm cấu hình mới. */
    ...
    /** Ngừng nhận hồ sơ mới nhưng không sửa Product snapshot của những hồ sơ đã submit. */
    ...
    /** Product ACTIVE phải được deactivate trước để tránh lưu trữ nhầm sản phẩm đang được công bố. */
    ...
    /** Bảo đảm số tiền và kỳ hạn borrower chọn nằm trong biên cấu hình đã công bố. */
    ...
    /** So sánh optimistic version do client gửi; chỉ domain transition được gọi kiểm tra này. */
    ...
    private void applyFinancialConfiguration(
        ...
        if (this.name.length() > 150) {
            throw LoanDomainException.invalidInput("LOAN_PRODUCT_NAME_TOO_LONG", "Tên sản phẩm tối đa 150 ký tự");
        }
    ...
    private LoanDomainException invalidTransition(LoanProductStatus target) {
    ...
    private static String requireText(String value, String code, String message) {
        if (value == null || value.isBlank()) {
            throw LoanDomainException.invalidInput(code, message);
        }
        return value.trim();
    }
