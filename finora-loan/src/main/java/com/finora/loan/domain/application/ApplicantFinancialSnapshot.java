...
public class ApplicantFinancialSnapshot {
    ...
    public static ApplicantFinancialSnapshot capture(
        ...
        if (declaredMonthlyIncome == null || declaredMonthlyIncome.signum() <= 0) {
            throw LoanDomainException.invalidInput("DECLARED_MONTHLY_INCOME_INVALID", "Thu nhập tháng phải lớn hơn 0");
        }
        if (employmentLengthMonths != null && employmentLengthMonths < 0) {
            throw LoanDomainException.invalidInput("EMPLOYMENT_LENGTH_INVALID", "Thâm niên làm việc không được âm");
        }
        if (homeOwnership == null) {
            throw LoanDomainException.invalidInput("HOME_OWNERSHIP_REQUIRED", "Phải chọn tình trạng nhà ở");
        }
        if (monthlyDebtObligations == null || monthlyDebtObligations.signum() < 0) {
            throw LoanDomainException.invalidInput("MONTHLY_DEBT_OBLIGATIONS_INVALID", "Nghĩa vụ nợ tháng không được âm");
        }
