...
public class FineractCommand {
    ...
    /** Ghi durable command và request snapshot trước external call để worker có thể tiếp tục sau restart. */
    ...
    /**
     * Cấp quyền thực thi cho một attempt. PROCESSING chỉ được vào đây sau khi state service đã
     * xác nhận lease cũ hết hạn dưới DB lock; cho phép trạng thái này để command không mắc kẹt sau restart.
     */
    public void markProcessing(Instant now) {
        if (status != FineractCommandStatus.PENDING
    ...
    /** Lưu response đã lọc để đối soát; credential và payload nhạy cảm không được ghi vào snapshot. */
    ...
    /** Lỗi tạm thời giữ nguyên command và idempotency key, chỉ thay thời điểm được thử lại. */
    ...
    /** Lỗi vĩnh viễn dừng auto retry nhưng giữ evidence để admin chẩn đoán. */
    ...
    private void requireProcessing() {
        if (status != FineractCommandStatus.PROCESSING) {
            throw new IllegalStateException("Command không ở trạng thái PROCESSING: " + status);
        }
    }
